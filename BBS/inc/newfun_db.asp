...
function chk_dbfile(chk_date,chktype)
	...
	if len(chk_date)>0 then
		...
		if chk_num(err)<>0 then 
			err.clear
			call dperror(true,"系统不能建立fso对象，请确保你的空间支持fso","")
		end if
		if chktype=1 then
...
sub do_dbbackup(backup_type,can_dbcounts,bk_date)
	...
	if can_dbcounts=0 then can_dbcounts=5
		...
		if chk_num(err)<>0 then 
			err.clear
			call dperror(true,"无法建立fso对象，请确保你的空间支持fso","")
		end if	
		backup_date_do=backup_date
		backup_date_do=server.mappath(DB_folder&"/"&backup_date_do)
		if backup_type=2 then
			'call dperror(objfso.fileexists(backup_date_do&"\"&DB_backupname),"今天已经计算过奖金，每天只能计算一次","")
		else
			'如果存在该备份目录，则不能备份
			call dperror(objfso.fileexists(backup_date_do&"\"&DB_backupname),"该日期已经有备份，请先删除再备份","")
		end if
		'最多只能有10个备份,其他的删除
...
sub db_dbrecovery(backup_date)
	...
	'if chk_num(err)<>0 then 
		'err.clear
		'call dperror(true,"","不能建立fso对象，请确保你的空间支持fso")
	'end if	
	...
	call dperror(not(objfso.fileexists(do_backup_date)),"","该日期未有备份，请检查")						
	...
	if not rs_backup.eof then
		...
		call dperror(true,"","系统中存在备份序号为"& old_backup_id &"的数据库["& old_backup_date &"]，比当前需要恢复的数据库还要新，请先删除该数据库，才能进行恢复")
	end if
